#!/usr/bin/env python3
"""Module that predicts objects in images using the Yolo model."""
import cv2
import glob
import os
import numpy as np
...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """Displays the image with boundary boxes, class names, and scores.
    ...
    def predict(self, folder_path):
        """Predicts objects in all images from a folder.

        Args:
            folder_path: string representing the path to the folder
                holding the images to predict.
        ...
        for i in range(len(images)):
            single_outputs = [out[i] for out in outputs]
            ...
            self.show_boxes(
                images[i], boxes, box_classes, box_scores, file_name
            )
